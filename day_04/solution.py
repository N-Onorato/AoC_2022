...
def main():
    ...
    print(len(list(filter(fullyContained, assignment_pairs))), sep='\n')

def fullyContained(pair: Tuple[AssignmentRange, AssignmentRange]) -> bool:
    first, second = pair if pair[0].length > pair[1].length else (pair[1], pair[0])
    return first.mininmum <= second.mininmum and first.maximum >= second.maximum
